...
import com.sms.server.api.stream.support.DynamicPlayItem;
import com.sms.server.api.stream.support.SimplePlayItem;
import com.sms.server.net.rtmp.BaseRTMPHandler;
...
public class StreamService implements IStreamService {
	...
	public void closeStream() {
		closeStream(SMS.getConnectionLocal(), getCurrentStreamId());
	}
    ...
    public void initStream(int streamId) { 
		...
		if (conn instanceof IStreamCapableConnection) {
			...
			logger.warn("ERROR in intiStream, connection is not stream capable");
		}
	}
			...
			if (reset instanceof Integer) {
				...
				switch (value) {
					case 0:
						...
						IStreamCapableConnection streamConn = (IStreamCapableConnection) SMS.getConnectionLocal();
	...
	public void publish(String name, String mode) {
		...
		if (conn instanceof IStreamCapableConnection) {
			...
			if (bsScope != null && !bsScope.getProviders().isEmpty()) {
				...
				IClientBroadcastStream bs = (IClientBroadcastStream)bsScope.getAttribute(IBroadcastScope.STREAM_ATTRIBUTE);
				closeStream(bs.getConnection(), bs.getStreamId());
			...
			try {
				...
				IProviderService providerService = (IProviderService) scope.getContext().getService(ScopeContextBean.PROVIDERSERVICE_BEAN);//(IProviderService) context.getBean(IProviderService.BEAN_NAME);
