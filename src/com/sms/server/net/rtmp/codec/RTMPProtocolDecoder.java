...
import com.sms.io.amf.AMF;
import com.sms.io.amf.Input;
import com.sms.io.amf.Output;
import com.sms.io.amf3.AMF3;
import com.sms.io.object.DataTypes;
...
import com.sms.server.net.rtmp.RTMPUtils;
import com.sms.server.net.rtmp.event.Abort;
...
import com.sms.server.net.rtmp.message.SharedObjectTypeMapping;
import com.sms.server.net.rtmp.message.StreamAction;
import com.sms.server.net.rtmp.protocol.HandshakeFailedException;
import com.sms.server.net.rtmp.protocol.ProtocolException;
...
public class RTMPProtocolDecoder implements Constants, IEventDecoder {
	...
	public ISharedObjectMessage decodeFlexSharedObject(IoBuffer in, RTMP rtmp) {
		...
		Input input;
		if (encoding == 0) {
			input = new com.sms.io.amf.Input(in);
		} else if (encoding == 3) {
			input = new com.sms.io.amf3.Input(in);
		} else {
	...
	public ISharedObjectMessage decodeSharedObject(IoBuffer in, RTMP rtmp) {
		final Input input = new com.sms.io.amf.Input(in);
		String name = input.getString();
		// Read version of SO to modify
		int version = in.getInt();
		// Read persistence informations
	...
	protected void doDecodeSharedObject(SharedObjectMessage so, IoBuffer in, Input input) {
		// Parse request body
		Input amf3Input = new com.sms.io.amf3.Input(in);
			...
			if (type == ISharedObjectEvent.Type.CLIENT_STATUS) {
				...
				if (length > 0) {
					...
					if (length > key.length() + 2) {
						...
						Input propertyInput;
						if (objType == AMF.TYPE_AMF3_OBJECT && !(input instanceof com.sms.io.amf3.Input)) {
	...
	public Notify decodeStreamMetadata(IoBuffer in, RTMP rtmp) {
		...
		if (rtmp.getEncoding() == Encoding.AMF0 || amfVersion != AMF.TYPE_AMF3_OBJECT) {
			input = new com.sms.io.amf.Input(copy);
		} else {
			com.sms.io.amf3.Input.RefStorage refStorage = new com.sms.io.amf3.Input.RefStorage();
			input = new com.sms.io.amf3.Input(copy, refStorage);
		}
		...
		if (dataType == DataTypes.CORE_STRING) {
			...
			if ("@setDataFrame".equals(setData)) {
				...
				log.debug("Unhandled request: {}", setData);
				if (log.isDebugEnabled()) {
					byte object = input.readDataType();
	...
	public FlexMessage decodeFlexMessage(IoBuffer in, RTMP rtmp) {
		...
		Input input = new com.sms.io.amf.Input(in);
		String action = Deserializer.deserialize(input, String.class);
		int invokeId = Deserializer.<Number> deserialize(input, Number.class).intValue();
			...
			while (in.hasRemaining()) {
				...
				switch (objectEncodingType) {
					...
					case AMF3.TYPE_VECTOR_OBJECT:
						// The next parameter is encoded using AMF3
						input = new com.sms.io.amf3.Input(in, refStorage);
						// Vectors with number and object have to have AMF3 forced
						((com.sms.io.amf3.Input) input).enforceAMF3();
						break;
