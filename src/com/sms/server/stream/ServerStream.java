...
public class ServerStream extends AbstractStream implements IServerStream, IFilter, IPushableConsumer, IPipeConnectionListener {
	...
	public void saveAs(String name, boolean isAppend) throws IOException, ResourceNotFoundException, ResourceExistException {
		try {
			...
			if (getScope().getContext().getBean(ScopeContextBean.FILECONSUMER_BEAN)!=null) {
				recordingFile = (FileConsumer) getScope().getContext().getBean(ScopeContextBean.FILECONSUMER_BEAN);
	...
	public void start() {
		...
		IProviderService providerService = (IProviderService) getScope().getContext().getService(ScopeContextBean.PROVIDERSERVICE_BEAN);
		...
		scheduler = QuartzSchedulingService.getInstance();
