...
import static com.sms.server.api.ScopeUtils.isApp;
import static com.sms.server.api.ScopeUtils.isRoom;
...
import com.sms.server.api.stream.IBroadcastStreamService;
import com.sms.server.api.stream.IClientBroadcastStream;
import com.sms.server.api.stream.IOnDemandStream;
import com.sms.server.api.stream.IOnDemandStreamService;
...
import com.sms.server.api.stream.ISubscriberStream;
import com.sms.server.api.stream.ISubscriberStreamService;
...
import com.sms.server.plugin.Plugin;
import com.sms.server.plugin.PluginDescriptor;
import com.sms.server.plugin.PluginRegistry;
...
import com.sms.server.stream.StreamService;
...
IStreamSecurityService, IStreamAwareScopeHandler {
	...
	private Set<IStreamAwareScopeHandler> streamListeners = new HashSet<IStreamAwareScopeHandler>();
	...
	public void addStreamListener(IStreamAwareScopeHandler streamListener) {
	...
	public void removeStreamListener(IStreamAwareScopeHandler streamListener) {
		...
		if (plugins != null) {
			for (PluginDescriptor desc : plugins) {
				...
				try {
					...
					IPlugin plugin = PluginRegistry.getPlugin(desc.getPluginName());
					...
					if (plugin instanceof Plugin) {
						...
						((Plugin) plugin).setApplication(this);
					}
					...
					if (desc.getMethod() != null) {
						...
						if (desc.getMethodReturnType() == null) {
							...
							if (returnClass instanceof IPluginHandler) {
	...
	public void measureBandwidth() {
		measureBandwidth(SMS.getConnectionLocal());
	}
	...
	public boolean createSharedObject(IScope scope, String name, boolean persistent) {
		ISharedObjectService service = SharedObjectService.getInstance();
	...
	public boolean hasBroadcastStream(IScope scope, String name) {
		IProviderService service = (IProviderService)scope.getContext().getService(ScopeContextBean.PROVIDERSERVICE_BEAN);
		return (service.getLiveProviderInput(scope, name, false) != null);
	}

	/** {@inheritDoc} */
	public IBroadcastStream getBroadcastStream(IScope scope, String name) {
		IStreamService service = (IStreamService) scope.getContext().getService(ScopeContextBean.STREAMSERVICE_BEAN);
	...
	public IOnDemandStream getOnDemandStream(IScope scope, String name) {
		...
		IOnDemandStreamService service = (IOnDemandStreamService) scope.getContext().getService(ScopeContextBean.STREAMSERVICE_BEAN);
	...
	public ISubscriberStream getSubscriberStream(IScope scope, String name) {
		log.warn("This won't work until the refactoring of the streaming code is complete.");
		ISubscriberStreamService service = (ISubscriberStreamService) scope.getContext().getBean(ScopeContextBean.SUBSCRIBERSTREAM_BEAN);
	...
	public String addScheduledJob(int interval, IScheduledJob job) {
		ISchedulingService service = QuartzSchedulingService.getInstance();
	...
	public String addScheduledCustomJob(Trigger trigger, IScheduledJob job) {
		...
		return service.addScheduledCustomJob(trigger, job);
	}
	...
	public double getStreamLength(String name) {
		double duration = 0;
		IProviderService provider = (IProviderService) scope.getContext().getService(ScopeContextBean.PROVIDERSERVICE_BEAN);
	...
	public void streamBroadcastClose(IBroadcastStream stream) {
		...
		long publishDuration = (SystemTimer.currentTimeMillis() - stream.getCreationTime()) / 1000;
		if (conn != null) {
			log
					.info(
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamBroadcastClose(stream);
		}
	}
	...
	public void streamBroadcastStart(IBroadcastStream stream) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamBroadcastStart(stream);
		}
	}
	...
	public void streamPlayItemPlay(ISubscriberStream stream, IPlayItem item, boolean isLive) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPlayItemPlay(stream, item, isLive);
		}
	}
	...
	public void streamPlayItemStop(ISubscriberStream stream, IPlayItem item) {
		...
		if (log.isInfoEnabled()) {
			...
			if (stream instanceof PlaylistSubscriberStream) {
				// converted to seconds
				playDuration = (SystemTimer.currentTimeMillis() - ((PlaylistSubscriberStream) stream).getCreationTime()) / 1000;
			}
			long playItemSize = -1;
			String playItemName = "";
			if (item != null) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPlayItemStop(stream, item);
		}
	}
	...
	public void streamPlayItemPause(ISubscriberStream stream, IPlayItem item, int position) {
		...
		log.info("W3C x-category:stream x-event:pause c-ip:{} x-sname:{}",
				SMS.getConnectionLocal().getRemoteAddress(), stream.getName());
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPlayItemPause(stream, item, position);
		}
	}

	public void streamPlayItemResume(ISubscriberStream stream, IPlayItem item, int position) {
		// log w3c connect event
		log.info("W3C x-category:stream x-event:unpause c-ip:{} x-sname:{}", SMS.getConnectionLocal()
				.getRemoteAddress(), stream.getName());
		
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPlayItemResume(stream, item, position);
		}
	}
	...
	public void streamPlayItemSeek(ISubscriberStream stream, IPlayItem item, int position) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPlayItemSeek(stream, item, position);
		}
	}

	public void streamPublishStart(IBroadcastStream stream) {
		// log w3c connect event
		IConnection connection = SMS.getConnectionLocal();
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamPublishStart(stream);
		}
	}
	...
	public void streamRecordStart(IBroadcastStream stream) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamRecordStart(stream);
		}
	}
	...
	public void streamSubscriberClose(ISubscriberStream stream) {
		...
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamSubscriberClose(stream);
		}
	}

	public void streamSubscriberStart(ISubscriberStream stream) {
		// log w3c connect event
		log.info("W3C x-category:stream x-event:play c-ip:{} x-sname:{}", SMS.getConnectionLocal().getRemoteAddress(),
				stream.getName());
		
		for (IStreamAwareScopeHandler listener : streamListeners) {
			listener.streamSubscriberStart(stream);
		}
	}
}
