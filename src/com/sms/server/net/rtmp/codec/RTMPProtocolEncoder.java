...
import com.sms.server.exception.ClientDetailsException;
import com.sms.server.net.rtmp.RTMPConnection;
import com.sms.server.net.rtmp.RTMPUtils;
import com.sms.server.net.rtmp.codec.RTMP.LiveTimestampMapping;
...
public class RTMPProtocolEncoder implements Constants, IEventEncoder {
	...
	protected boolean dropMessage(RTMP rtmp, int channelId, IRTMPEvent message) {
		...
		boolean drop = false;
		//FixedBy: plr
		//skip dropMessage in encoder,
		if(drop == false) return drop;
	...
	private int calculateHeaderSize(final RTMP rtmp, final Header header, final Header lastHeader) {
		...
		int timer ;
		if(headerType==HEADER_NEW) timer = header.getTimer();
		else timer = (int) RTMPUtils.diffTimestamps(header.getTimer(), lastHeader.getTimer());
		if (timer < 0 || timer >= 0xffffff) {
			if(headerType!=HEADER_CONTINUE){
				channelIdAdd+=4;
				log.info("add extend time!");
			}
		}
	...
	*/
	public void doEncodeSharedObject(ISharedObjectMessage so, RTMP rtmp, IoBuffer out) {
		final Output output = new com.sms.io.amf.Output(out);
		final Output amf3output = new com.sms.io.amf3.Output(out);
	...
	protected void encodeNotifyOrInvoke(IoBuffer out, Notify invoke, RTMP rtmp) {
		...
		Output output = new com.sms.io.amf.Output(out);
		...
		if (!isPending) {
			...
			if (rtmp.getEncoding() == Encoding.AMF3 && rtmp.getMode() == RTMP.MODE_CLIENT) {
				output = new com.sms.io.amf3.Output(out);
			}
		...
		if (call.getServiceName() == null && "connect".equals(call.getServiceMethodName())) {
			...
			output = new com.sms.io.amf.Output(out);
		} else {
			if (rtmp.getEncoding() == Encoding.AMF3) {
				output = new com.sms.io.amf3.Output(out);
			} else {
