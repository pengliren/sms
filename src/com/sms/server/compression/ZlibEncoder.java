...
/**
 * Compresses using the deflate algorithm.
 */
public class ZlibEncoder {
    ...
    public IoBuffer encode(IoBuffer msg) throws Exception {
        ...
        synchronized (z) {
            try {
                ...
                if (z.next_out_index != 0) {
                    result = IoBuffer.allocate(out.length).setAutoExpand(true);
                    result.put(out, 0, z.next_out_index);
                } else {
                    result = IoBuffer.allocate(0);
                }
            } finally {
    ...
    public IoBuffer finishEncode() {
    	
		if (!finished.compareAndSet(false, true)) {
			return IoBuffer.wrap(EMPTY_ARRAY);
		}
		...
		synchronized (z) {
            try {
                ...
                if (resultCode != JZlib.Z_OK && resultCode != JZlib.Z_STREAM_END) {
                	footer = IoBuffer.wrap(EMPTY_ARRAY);
                } else if (z.next_out_index != 0) {
                    footer = IoBuffer.allocate(out.length).setAutoExpand(true);
                    footer.put(out, 0, z.next_out_index);
                } else {
